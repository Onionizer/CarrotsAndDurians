...
function CalcList(props) {
  ...
  /*
    ...
    (Would set the item at props.calcList[2] to have interval 10 while keeping the
    other values the same)
  */
  function updateCalc(index, newData) {
    const oldData = props.calcList[index]; // Get the current object for this item
    const newCalc = {
        ...oldData, // include the old values in the new object
        ...newData, // update the value with the new given value
    };
    props.onUpdate([
      ...props.calcList.slice(0, index), // include items before this one
      newCalc, // include this item
      ...props.calcList.slice(index + 1), // include items after
    ]);
  }
