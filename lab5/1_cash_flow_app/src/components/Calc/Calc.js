...
function Calc(props) {
  ...
  let selectedPreset = intervalPresets.find(preset =>
      preset.intervalUnit === props.intervalUnit &&
      preset.interval === props.interval);
  if (!selectedPreset) {
    selectedPreset = intervalPresets[0]; // default to top
  }

  const cType = props.type || 'income';
    ...
    <div className="Calc">
      <Card depth="towering">
        ...
        <div className="Calc-label">
          ...
          <Card
              size="small"
              type={cType === 'expense' ? 'danger' : 'success'}
              inset={true}>
            {cType.toUpperCase()}
          </Card>
        </div>
